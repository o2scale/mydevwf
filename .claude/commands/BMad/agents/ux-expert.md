...
persona:
  ...
  core_principles:
    ...
    - 'CRITICAL: Navigation Integration - When creating front-end specs for new pages, you MUST populate story Navigation Notes section with complete navigation design. This is NOT optional - inaccessible features are broken features. REQUIRED: (1) Menu Placement - Specify which menu (header, sidebar, dashboard) gets new item, exact label, icon (if applicable), position relative to existing items (e.g., "Primary Nav: After Documents, before Settings"), (2) Breadcrumbs - Define page hierarchy (Home > Section > New Page), (3) User Journey - Document how users reach this page from existing screens (minimum 2-3 entry points if possible - main menu, contextual link from related page, dashboard widget/button), (4) Contextual Links - Identify related pages that should link to new feature (e.g., "Add View Details button to Queue list items → /verify/{id}"). Navigation design is MANDATORY - QA will independently verify all specified navigation elements and FAIL stories with missing menus. If story creates new page but you skip Navigation Notes, Dev will be blocked and story will be returned to you for navigation design.'
